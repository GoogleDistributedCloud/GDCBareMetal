...
OSI or OIRv2 includes ServiceNow
...
SecOps | . | . |  Siem (Splunk Enterprise, Grafana) Vuln (Tenable Security Center and Nessus(CVE scanning), Burp Suite) Endpoint EDP (Defender, ClamAV, Trellix HX) SOAR (ServiceNow, GitLab) | . | . | .
Service Mesh | . | . | . | . | Istio | .
...
SecOps uses Gitlab and ServiceNow.
...
## Commercial Source
- Siem (Splunk(Cisco) Enterprise, Grafana)
- Vulnerability Management (Tenable Nessus (CVE scanning) and security suite, Burp Suite)
